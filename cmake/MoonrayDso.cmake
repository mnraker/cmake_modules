...
function(moonray_get_rdl2_json_exporter_command outCommand outDependency)
    if(TARGET rdl2_json_exporter)
        set(${outCommand} "$<TARGET_FILE:rdl2_json_exporter>" PARENT_SCOPE)
        set(${outDependency} rdl2_json_exporter PARENT_SCOPE)
    elseif(TARGET SceneRdl2::rdl2_json_exporter)
        get_target_property(rdl2_json_exporter SceneRdl2::rdl2_json_exporter IMPORTED_LOCATION)
        if(NOT rdl2_json_exporter)
            get_target_property(rdl2_json_exporter SceneRdl2::rdl2_json_exporter IMPORTED_LOCATION_RELEASE)
        endif()
        set(${outCommand} "${rdl2_json_exporter}" PARENT_SCOPE)
...
function(moonray_add_rdl2_json_exporter_dependency target)
    ...
    elseif(NOT TARGET SceneRdl2::rdl2_json_exporter)
...
function(moonray_dso_simple targetName)
   ...
   if (NOT ARG_TEST_DSO)
    ...
    if(IsWindowsPlatform)
        ...
        add_custom_command(OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/${dsoName}.json
            POST_BUILD
            COMMAND ${RDL2_JSON_EXPORTER_COMMAND} --dso_path ${CMAKE_CURRENT_BINARY_DIR}/${configDir}
            --in $<TARGET_FILE:${targetName}_proxy>
            --out ${CMAKE_CURRENT_BINARY_DIR}/${dsoName}.json
            DEPENDS
                ${targetName}_proxy
...
function(moonray_ispc_dso name)
    ...
    if(DEFINED Python_EXECUTABLE AND NOT "${Python_EXECUTABLE}" STREQUAL "")
        set(_ispc_dso_python "${Python_EXECUTABLE}")
    elseif(DEFINED Python3_EXECUTABLE AND NOT "${Python3_EXECUTABLE}" STREQUAL "")
        set(_ispc_dso_python "${Python3_EXECUTABLE}")
    elseif(DEFINED PYTHON_EXECUTABLE AND NOT "${PYTHON_EXECUTABLE}" STREQUAL "")
        set(_ispc_dso_python "${PYTHON_EXECUTABLE}")
    else()
        set(_ispc_dso_python python)
    endif()
    ...
    add_custom_command(
        ...
        COMMAND
            ${_ispc_dso_python} ${ISPC_DSO_GEN_SCRIPT} ${jsonSrc}
